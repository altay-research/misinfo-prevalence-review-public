...
Every paper's text is in `texts/<item_id>.txt`. Read the paper. Do not use the web, and do not read
any file outside this folder.

## STOP AFTER ONE FILE
Code the single batch file you were given, save it in place, and stop. Long sessions degrade:
in an earlier campaign a coder reproduced only 45% of its own codes late in a 647-row session.

---

## Task A — `verify_NN.csv` (our extraction is shown; check it)
...
## Task B — `blind_NN.csv` (our construct is NOT shown; code it yourself)

Each row gives only the paper, the quantity as reported, the denominator sentence and how the paper
defined misinformation. Fill `YOUR_construct` and `YOUR_confidence` (1-5). Do not look for our code.
